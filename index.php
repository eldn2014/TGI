...
if (isset($_POST['email']) || isset($_POST['password'])) {
    ...
    } elseif (strlen($_POST['password']) == 0)  // verifica de o campo senha ta preenchido
    {
        ...
        $sql_code = "SELECT * FROM users as c 
                        RIGHT join users_status as d on d.ID_conta = c.ID_conta 
                        RIGHT join validação_status as e on e.ID_Status = d.Status_curso 
                        WHERE c.Email = '$email' ;";
            ...
            <form class="meu-form" method="POST">
                ...
                <div class="formulario-digitaveis">
                    <label for="email">E-mail:
                        <input type="email" id="email" name="email" autocomplete="off" placeholder="Digite seu E-mail" required>
                    </label>
                </div>
                ...
                <div class="formulario-digitaveis">
                    <label for="Senha">Senha:
                        <input type="password" id="password" name="password" placeholder="Digite sua senha" title="Mínimo de 8 caracteres contendo pelo menos 1 letra maiuscula e 1 número" pattern="^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$" required>
                    </label>
                </div>
